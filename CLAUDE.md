...
- `electron/speech.ts` · `framing.ts` · `broll.ts` · `takes.ts` · `playable.ts`: pure logic, no Electron, each with its own `npm run test:*` suite. Put decisions here, not in the handlers
