...
const TOOLS = [
  ...
  { name: 'add_media', description: 'Import video/audio/image media by absolute path, including WebM, animated WebP, and PNG frame-sequence ZIPs, then (by default) append it to the timeline. chromaKey removes a keyed background.', inputSchema: { type: 'object', properties: { path: str, place: { ...bool, description: 'false = bin only' }, start: { ...num, description: 'explicit timeline position (s)' }, chromaKey: { ...str, description: 'hex key colour to remove, e.g. #00e800' } }, required: ['path'] } },
  ...
  { name: 'set_format', description: 'Set export format: orientation landscape|portrait|square, resolution 4K|1440p|1080p|720p, fps 24|30|60, fileFormat mp4|webm|webp|png-sequence, and opaque background #RRGGBB.', inputSchema: { type: 'object', properties: { orientation: str, resolution: str, fps: num, fileFormat: { ...str, enum: ['mp4', 'webm', 'webp', 'png-sequence'] }, background: str } } },
  { name: 'export_video', description: 'Render the timeline at absolute outputPath. Extension selects MP4, WebM, animated WebP with alpha, or a transparent PNG-frame ZIP. WebP/PNG have no audio. Blocks until done; video outputs can return the automatic quality check.', inputSchema: { type: 'object', properties: { outputPath: str, format: { ...str, enum: ['mp4', 'webm', 'webp', 'png-sequence'] }, background: str, qualityCheck: bool }, required: ['outputPath'] } },
