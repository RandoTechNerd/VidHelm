...
- **Adversal AI** (`adversal.ai`): a third-party video-analysis MCP: the agent submits a video and gets back Markdown notes, chapters, and extracted stills (async, 100 free min/month, Python 3.13+). Great for auto-chapters and finding highlights in long source footage before cutting. Setup: `pip install adversal-cli`, then `claude mcp add adversal -- adversal-cli`, then ask your assistant to call its `authenticate` tool once, which opens a browser sign-in.

  VidHelm meets it halfway with the **`prepare_analysis`** tool. Ask your assistant to "prepare the project for analysis" and it either flattens the timeline to a small mp4 (whose timestamps line up with your timeline exactly) or hands over the original clip with in and out points, no re-render needed. The same call returns `gaps`: the stretches of your video with no tag point nearby. Analysing only those on a repeat pass means footage you have already marked is left alone, which also keeps you inside the free tier. Two things worth knowing: the video is uploaded to their servers, unlike everything else in VidHelm, and the notes describe the file you sent, so final chapter times should be read back off the finished timeline.
