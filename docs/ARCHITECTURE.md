...
- **Media import** (`classifyMedia` in App.tsx): extension tables are only a hint - `get-metadata` (ffprobe) has the final say, so unusual-but-valid files import and broken ones are refused with a reason. Animated WebP and PNG-frame ZIPs take one opt-in normalization path (`electron/alphaImport.ts`): Sharp/fflate decode their frames, FFV1/BGRA becomes the lossless export master, and VP9 with `ALPHA_MODE=1` becomes the Chromium preview. The cache key includes source path, size, and mtime. Junk demuxers are filtered explicitly: ffprobe happily reads a `.txt` as an `ansi` video stream via the `tty` demuxer, which used to land on the timeline as a 0.04s clip. 3D models bypass the bin and open the 3D Studio; `.html` pages are scanned for a model first (`electron/modelSniff.ts`: sibling file reference, base64 payload, or inline model text; Electron-free so it can be tested standalone).
